...
export function PhotographerAuth({ onLogin, onBack }: PhotographerAuthProps) {
  ...
  const curTab = location.pathname.includes("register") ? "register" : "login";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch("http://127.0.0.1:8000/api/dang-nhap", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
        },
        credentials: "include", // 🔥 thêm dòng này để Laravel nhận session cookie (nếu dùng sanctum)
        body: JSON.stringify({
  ...
  localStorage.setItem("user_info", JSON.stringify(data.user)); // 🔥 Lưu user thật
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/dang-ky", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
                    ...
                    <div className="w-full max-w-md mx-auto p-6 relative z-10">
                        ...
                        <Tabs
                            ...
                            onValueChange={(val) => {
                                val == "login"
                                    ? navigate("/photographer-auth-login")
                                    : navigate("/photographer-auth-register");
                            }}
                            className="w-full"
                        >
